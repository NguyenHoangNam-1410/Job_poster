...
<head>
  ...
  <style>
    ...
    .menu-section { font-weight: 700; padding: 0.75rem 1rem; color: #555; text-transform: uppercase; font-size: 0.75rem; }
    .menu-item { display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem 1rem; text-decoration: none; color: #0a4d5c; font-weight: 600; }
    .menu-item.active { background: #b4ff39; color: #0a4d5c; }
    .sidebar-footer { padding: 1rem; border-top: 1px solid #eee; display: flex; align-items: center; gap: 0.5rem; }

    /* Mobile toggle button */
    .toggle-sidebar { display: inline-flex; padding: 0.5rem; margin-left: auto; background: white; border-radius: 0.25rem; }
   @media(min-width:768px){ .toggle-sidebar, .sidebar, .sidebar-overlay { display: none !important; } }
  </style>
</head>
  ...
  <header class="y2k-header text-white">
    <div class="container mx-auto px-4 flex items-center justify-between">
      ...
      <nav class="hidden md:flex items-center ml-auto">
        ...
        <?php else: ?>
          <a href="/Job_poster/public/" class="y2k-nav-link <?= $_SERVER['REQUEST_URI'] === '/Job_poster/public/' ? 'active' : '' ?>">Home</a>
          <?php if ($_SESSION['user']['role'] == 'Employer'): ?>
              <a href="/Job_poster/public/company-profile" class="y2k-nav-link <?= strpos($_SERVER['REQUEST_URI'], '/company-profile') !== false ? 'active' : '' ?>">Company Profile</a>
              <a href="/Job_poster/public/my-jobs" class="y2k-nav-link <?= strpos($_SERVER['REQUEST_URI'], '/my-jobs') !== false ? 'active' : '' ?>">My Jobs</a>
              <a href="/Job_poster/public/my-feedbacks" class="y2k-nav-link <?= strpos($_SERVER['REQUEST_URI'], '/my-feedbacks') !== false ? 'active' : '' ?>">Feedbacks</a>
        ...
        <!-- Profile Section -->
        <?php if (isset($_SESSION['user'])): ?>
          <a href="/Job_poster/public/profile" 
            class="flex items-center space-x-2 y2k-nav-link transition p-2 rounded-lg">
          ...
          <a href="/Job_poster/public/logout" class="flex items-center space-x-1 text-sm font-semibold text-red-400 hover:text-red-600 hover:bg-red-100/20 px-3 py-1 rounded-lg transition">
            <span>Logout</span>
