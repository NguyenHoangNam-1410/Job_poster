...
class CompanyService {
    ...
    public function createCompanyProfile($userId, $data) {
        // Validate data
        if (empty($data['name'])) throw new Exception("Company name is required.");
        if (empty($data['contact'])) throw new Exception("Contact person is required.");
        if (empty($data['email']) || !filter_var($data['email'], FILTER_VALIDATE_EMAIL))
            throw new Exception("Valid email required.");
        if (empty($data['phone'])) throw new Exception("Contact phone required.");
        ...
        if (empty($data['logo'])) throw new Exception("Logo required.");
