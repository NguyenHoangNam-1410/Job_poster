...
if (form) {
  ...
  form.addEventListener("submit", (event) => {
    ...
    // Handle post_job and save_draft actions
    if (action === "post_job" || action === "save_draft") {
      const categorySelect = document.getElementById("jobCategoriesNew");
      const anyChecked = categorySelect
        ? Array.from(categorySelect.options).some((opt) => opt.selected)
        : true;

      if (!anyChecked && action === "post_job") {
        alert("Please select at least one job category.");
        event.preventDefault();
        return;
      }

      if (action === "post_job" && !validateDeadline()) {
...
function validateDeadline() {
  ...
  if (selectedDate > new Date(9999, 11, 31)) {
    deadlineInput.setCustomValidity(
      "Application deadline is too far in the future."
    );
    return false;
  }
